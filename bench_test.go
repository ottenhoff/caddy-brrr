...
func benchmarkEncode(b *testing.B, payload []byte, encoding encode.Encoding) {
	...
	ratio := encoderCompressionRatio(b, payload, encoding)
...
func benchmarkEncodeHandler(b *testing.B, enc *encode.Encode, encCase encoderCase, corpus benchmarkCorpus) {
	...
	ratio := handlerCompressionRatio(b, enc, encCase, corpus, next)
...
func encoderCompressionRatio(b *testing.B, payload []byte, encoding encode.Encoding) float64 {
	b.Helper()
	...
	return compressionRatio(len(payload), dst.Len())
}

func handlerCompressionRatio(b *testing.B, enc *encode.Encode, encCase encoderCase, corpus benchmarkCorpus, next caddyhttp.Handler) float64 {
	b.Helper()
	...
	return compressionRatio(len(corpus.data), w.body.Len())
}
